...
#[test]
fn parses_stable_item_cursor_after_cursor_schema_upgrade() {
    let thread_id = ThreadId::default();
    let cursor = format!(
        r#"{{"threadId":"{thread_id}","scope":{{"kind":"items"}},"rolloutOrdinal":42,"includeAnchor":true}}"#
    );

    let parsed = parse_cursor(
        Some(cursor.as_str()),
        thread_id,
        CursorScope::ItemsByCreatedAtOrdinal,
    )
    .expect("parse stable cursor");
...
#[tokio::test]
async fn search_occurrences_maps_unicode_lowercase_expansion_to_original_range() {
    let (_home, store, thread_id) = store_with_mode(ThreadHistoryMode::Paginated).await;
    let db = history_db(&store).await;
    insert_turn(
        db,
        ...
        "completed",
        /*error_json*/ None,
        /*first_user_item_id*/ Some("İstanbul"),
        /*final_agent_item_id*/ None,
    )
    .await;
    insert_item(
    ...
    let occurrences = store
        .search_thread_occurrences(SearchThreadOccurrencesParams {
            thread_id,
            search_term: "i".to_string(),
            cursor: None,
            page_size: 1,
        })
        .await
        .expect("search Unicode lowercase expansion");

    let occurrence = occurrences.items.first().expect("Unicode match");
    assert_eq!(occurrence.item_id, "İstanbul");
    assert_eq!(occurrence.snippet, "İstanbul");
    assert_eq!(
        occurrence.snippet_match_range,
        SearchTextRange { start: 0, end: 1 }
    );
}
