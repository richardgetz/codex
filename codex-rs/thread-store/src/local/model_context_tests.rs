...
async fn fork_context_excludes_items_after_frozen_cutoff() {
    ...
    let context = load_for_fork(&store, thread_id, lineage, Some(history_base))
        .await
...
#[tokio::test]
async fn retries_lineage_scan_after_rollout_compression_replaces_plain_file() {
    let home = TempDir::new().expect("temp dir");
    let uuid = Uuid::from_u128(/*v*/ 1008);
    let thread_id = ThreadId::from_string(&uuid.to_string()).expect("thread id");
    let expected_items = [
        turn_started("turn-1"),
        user_message("message preserved across compression"),
    ];
    let path = write_ordinaled_paginated_rollout(
        home.path(),
        "2025-01-03T13-00-07",
    ...
    let lineage = store
        ...
        .await
        .expect("resolve plain lineage");
    let session_meta = codex_rollout::read_session_meta_line(path.as_path())
        .await
        .expect("read session metadata");
    compress_rollout(path.as_path());

    let context = scan_model_context_from_lineage_with_retry(
        &store,
        thread_id,
        lineage,
        session_meta.clone(),
    ...
    .await
    .expect("retry scan after representation transition");

    let expected = std::iter::once(RolloutItem::SessionMeta(session_meta))
        .chain(expected_items)
        .collect::<Vec<_>>();
    assert_eq!(
...
#[tokio::test]
async fn retry_keeps_requested_thread_metadata_when_cutoff_is_in_compressed_ancestor() {
    let home = TempDir::new().expect("temp dir");
    let root_uuid = Uuid::from_u128(/*v*/ 1009);
    let root_id = ThreadId::from_string(&root_uuid.to_string()).expect("root id");
    let root_items = [
        turn_started("root-turn"),
        user_message("root message preserved across compression"),
    ];
    ...
    let lineage = store
        ...
        .await
        .expect("resolve plain nested lineage");
    ...
    .await
    .expect("retry nested lineage scan after ancestor representation transition");

    let expected = std::iter::once(RolloutItem::SessionMeta(child_meta))
        .chain(root_items)
        .collect::<Vec<_>>();
    assert_eq!(
        serde_json::to_value(context).expect("serialize context"),
