...
pub fn select_user_messages(
    ...
    let Some((latest, middle)) = remaining.split_last() else {
        return selection;
    };
    // The latest message is an anchor too, including when it cannot fit. It may
    // be the message that grants or revokes authorization.
    selection.indices.push(latest.index);
    selection.tokens = selection.tokens.saturating_add(latest.tokens);
    for message in middle.iter().rev() {
        let tokens = selection.tokens.saturating_add(message.tokens);
