...
impl FileSystemAccessMode {
    pub fn can_read(self) -> bool {
        !matches!(
            self,
            FileSystemAccessMode::None | FileSystemAccessMode::Deny
        )
    }
...
pub enum FileSystemPath {
    ...
    /// explicit read-deny access modes only.
    GlobPattern {
        pattern: String,
    },
    Special {
...
impl FileSystemSandboxPolicy {
    ...
    pub fn preserve_deny_read_restrictions_from(&mut self, existing: &Self) {
        let has_deny_read_entries = existing
            .entries
            .iter()
            .any(|entry| !entry.access.can_read());
            ...
            && self.entries.iter().any(|entry| {
                ...
                match &entry.path {
                    ...
                    FileSystemPath::Special { value } => match value {
                        FileSystemSpecialPath::Root => !entry.access.can_read(),
    ...
    /// Compatibility alias for callers that still use the pre-context local
    /// path API. New code should use [`Self::resolve_access_for_local_path_with_cwd`].
    pub fn resolve_access_with_cwd(&self, path: &Path, cwd: &Path) -> FileSystemAccessMode {
        self.resolve_access_for_local_path_with_cwd(path, cwd)
    }

    /// Compatibility alias for callers that still use the pre-context local
    /// path API. New code should use [`Self::can_read_local_path_with_cwd`].
    pub fn can_read_path_with_cwd(&self, path: &Path, cwd: &Path) -> bool {
        self.can_read_local_path_with_cwd(path, cwd)
    }

    /// Compatibility alias for callers that still use the pre-context local
    /// path API. New code should use [`Self::can_write_local_path_with_cwd`].
    pub fn can_write_path_with_cwd(&self, path: &Path, cwd: &Path) -> bool {
        self.can_write_local_path_with_cwd(path, cwd)
    }
...
mod tests {
    ...
    #[test]
    fn deny_mode_entries_are_preserved_as_read_denies() {
    ...
    #[test]
    fn deny_mode_globs_are_returned_as_unreadable_globs() {
        let policy = FileSystemSandboxPolicy::restricted(vec![FileSystemSandboxEntry {
            path: FileSystemPath::GlobPattern {
        ...
        assert_eq!(
            policy.get_unreadable_globs_with_cwd(Path::new("/tmp/project")),
            vec!["/tmp/project/secrets/**/*.json".to_string()]
        );
    }
