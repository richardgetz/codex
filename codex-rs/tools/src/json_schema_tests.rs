...
fn parse_tool_input_schema_infers_object_shape_and_defaults_properties() {
    ...
    // - The child property has only a description, so it is treated as a
    //   string schema and preserves the description.
...
fn parse_tool_input_schema_coerces_unrecognized_object_schema_to_empty_schema() {
    ...
    //
    // Expected normalization behavior:
    // - Object schemas with only a description are treated as string schemas
    //   and preserve the description.
    let schema = parse_tool_input_schema(&serde_json::json!({
        "description": "Ticket identifier",
        "title": "Ticket ID"
    }))
