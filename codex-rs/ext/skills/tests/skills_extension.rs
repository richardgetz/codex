...
#[tokio::test]
async fn plain_name_selection_prefers_executor_skill_over_host_name_collision() -> TestResult {
    let read_requests = Arc::new(Mutex::new(Vec::new()));
    ...
    assert!(
        fragments[1]
            .render()
            .contains("<path>skill://executor/lint-fix/SKILL.md</path>")
    );
    assert_eq!(
        vec![(
            SkillAuthority::new(SkillSourceKind::Executor, "env-1"),
