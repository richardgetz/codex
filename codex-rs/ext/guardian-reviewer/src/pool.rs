...
pub trait ReviewerSession: Send + Sync + 'static {
    ...
    /// Stops a test-owned session when the pool's runtime is shut down.
    /// Production sessions normally use ThreadManager lifecycle cleanup instead.
    fn shutdown(&self) -> impl Future<Output = ()> + Send {
        async {}
    }

    /// Returns the durable rollout path for host diagnostics and test inspection.
...
impl<S: ReviewerSession> ReviewerPool<S> {
    ...
    ///
    /// The runtime tracker also joins the test session on pool shutdown so tests can
    /// exercise the same cancellation boundary as a managed reviewer.
    #[doc(hidden)]
    pub async fn cache_for_test(&self, session: S) {
        ...
        let session_cancellation = self.runtime.cancellation.child_token();
        *self.trunk.lock().await = Some(Arc::new(Trunk {
            session,
    ...
    /// Returns the durable rollout path of the current trunk, when one exists.
