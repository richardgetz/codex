...
impl OnboardingScreen {
    pub(crate) async fn new(tui: &mut Tui, args: OnboardingScreenArgs) -> Self {
        ...
        // The trust prompt may be entered from startup or from an already-running
        // session. Read effective server configuration so managed sandbox state is
        // reflected consistently in both paths.
