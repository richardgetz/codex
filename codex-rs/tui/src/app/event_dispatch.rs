    ...
    pub(crate) async fn handle_event(
        ...
        match event {
            ...
            AppEvent::UserVerificationApproved {
                ...
                Box::pin(self.start_user_verification(app_server, thread_id, server_name, request_id))
                    .await?;
            }
            ...
            AppEvent::SwitchAccount { alias, reason } => {
                let previous_alias = self.config.accounts.active.clone();
                match app_server.switch_account(alias.clone()).await {
                    Ok(outcome) => {
                        ...
                        if outcome.needs_auth_selection
                            && matches!(reason, crate::app_event::AccountSwitchReason::AutoRotation)
                        {
                            let previous_label =
                                previous_alias.as_deref().unwrap_or("default").to_string();
                            match app_server.switch_account(previous_alias.clone()).await {
                                Ok(rollback_outcome) => {
                                    self.config.accounts.active = previous_alias.clone();
                                    self.chat_widget.set_active_account_alias(previous_alias);
                                    self.chat_widget
                                        .mark_account_rotation_alias_unavailable(alias.as_deref());
                                    self.chat_widget.add_error_message(format!(
                                        "Account alias {label} has no saved credentials; staying on account alias {previous_label}. Use `/account {label}` to choose how to authenticate it."
                                    ));
                                    if let Some(err) = rollback_outcome.account_status_error {
                                        self.chat_widget.add_error_message(err);
                                    }
                                }
                                Err(rollback_err) => {
                                    ...
                                    self.chat_widget.add_error_message(format!(
                                        "Account alias {label} has no saved credentials, and restoring account alias {previous_label} failed: {rollback_err}"
                                    ));
                                }
                            }
                                        ...
                                        crate::onboarding::onboarding_screen::run_onboarding_app(
                                            crate::onboarding::onboarding_screen::OnboardingScreenArgs {
                                                ...
                                                bedrock_setup_enabled:
                                                    crate::should_show_bedrock_setup_wizard(
                                                        crate::LoginStatus::NotAuthenticated,
                                                        self.chat_widget.requires_openai_auth,
                                                        &self.config,
                                                        &self.app_server_target,
                                                    ),
            ...
            AppEvent::TeamWorkPolicyUpdateTimeout {
                thread_id,
                request_id,
            } => {
                if self.active_thread_id == Some(thread_id) {
                    self.chat_widget
                        .on_team_work_policy_update_timeout(request_id);
                }
            }
            ...
            AppEvent::UpdatePermissionProfile(permission_profile) => {
                ...
                {
                    tracing::warn!(%err, "failed to set permission profile on app config");
                    self.chat_widget
                        .add_error_message(format!("Failed to set permission profile: {err}"));
                    return Ok(AppRunControl::Continue);
                }
                ...
                self.config = config;
                let permission_profile = self.config.permissions.permission_profile().clone();
                if let Err(err) = self
                    .chat_widget
                    .set_permission_profile_from_session_snapshot(
                        PermissionProfileSnapshot::legacy(permission_profile.clone()),
                    )
                {
                    tracing::warn!(%err, "failed to set permission profile on chat config");
                    self.chat_widget
