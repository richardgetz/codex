...
impl ChatWidget {
    ...
    fn open_plan_implementation_prompt(&mut self) {
        let default_mask = collaboration_modes::default_mode_mask_with_config(
    ...
    fn new_with_op_target(common: ChatWidgetInit, codex_op_target: CodexOpTarget) -> Self {
        ...
        let mut widget = Self {
            ...
            config,
            previous_custom_permission_selection: None,
    ...
    fn replay_initial_messages(&mut self, events: Vec<EventMsg>) {
        for msg in events {
            self.handle_event_msg_for_tests(msg);
        }
    }

    #[cfg(test)]
    pub(crate) fn handle_codex_event(&mut self, event: Event) {
        self.handle_event_msg_for_tests(event.msg);
    }

    #[cfg(test)]
    fn handle_event_msg_for_tests(&mut self, msg: EventMsg) {
        match msg {
        ...
        match msg {
            ...
            EventMsg::TurnAborted(ev) => match ev.reason {
                ...
                ProtocolTurnAbortReason::BudgetLimited => {
                    ...
                    self.on_interrupted_turn(TurnAbortReason::BudgetLimited);
                }
            },
            EventMsg::PlanUpdate(update) => self.on_plan_update(update),
            EventMsg::ScratchpadUpdate(update) => self.on_scratchpad_update(update),
    ...
    pub(crate) fn open_collaboration_modes_popup(&mut self) {
        let collaboration_modes_config = self.config.collaboration_modes_config();
    ...
    pub(crate) fn open_plan_reasoning_scope_prompt(
        ...
        let plan_reasoning_source =
            ...
            } else if let Some(plan_mask) = collaboration_modes::plan_mask_with_config(
                self.model_catalog.as_ref(),
                self.config.collaboration_modes_config(),
            ) {
    ...
    pub(crate) fn open_permissions_popup(&mut self) {
        ...
        let restorable_current_selection = self.restorable_current_permission_selection();
        ...
        for preset in presets.into_iter() {
            ...
            let default_actions: Vec<SelectionAction> = if requires_confirmation {
                ...
                Self::with_restorable_permission_action(
                    ...
                    restorable_current_selection.clone(),
                )
            ...
            let disabled_reason = self
                ...
                .or_else(|| {
                    self.config
                        .permissions
                        .permission_profile
                        .can_set(&previous_selection.permission_profile)
                        .err()
                        .map(|err| err.to_string())
                });
            items.push(SelectionItem {
    ...
    fn permission_selection_matches_current(
        ...
        selection.approval_policy
            == AskForApproval::from(self.config.permissions.approval_policy.value())
            && selection.permission_profile == self.config.permissions.permission_profile()
    ...
    fn should_show_plan_mode_nudge(&self) -> bool {
        ...
        self.collaboration_modes_enabled()
            && collaboration_modes::plan_mask_with_config(
                self.model_catalog.as_ref(),
                self.config.collaboration_modes_config(),
            )
            .is_some()
    ...
    fn initial_collaboration_mask(
        ...
        let mut mask = initial_mode
            .and_then(|mode| {
                collaboration_modes::mask_for_kind_with_config(
                    model_catalog,
                    mode,
