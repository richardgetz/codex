...
//! is in progress, while MCP server startup is in progress, and while a multi-agent wait call is
//! active. Those lifecycles are tracked independently and synchronized via
...
use crate::app_event::RestorablePermissionSelection;
...
pub(crate) use self::misalignment_policy::MisalignmentTurnSource;
...
fn token_usage_info_from_app_server(token_usage: ThreadTokenUsage) -> TokenUsageInfo {
    TokenUsageInfo {
        total_token_usage: TokenUsage {
            ...
            cache_write_tokens: token_usage.total.cache_write_input_tokens,
...
impl ChatWidget {
    ...
    pub(crate) fn mark_account_rotation_alias_unavailable(&mut self, alias: Option<&str>) {
        self.exhausted_account_rotation_aliases
            .insert(alias.unwrap_or("default").to_string());
    }
    ...
    pub(crate) fn remember_custom_permission_selection(
        ...
        self.previous_custom_permission_selection = Some(selection);
    }

    pub(crate) fn on_scratchpad_update_verbose(&mut self, update: ScratchpadUpdateEvent) {
        self.add_to_history(history_cell::new_scratchpad_update_verbose(update));
    }

    pub(crate) fn on_scratchpad_update(&mut self, update: ScratchpadUpdateEvent) {
    ...
    pub(crate) fn handle_codex_event(&mut self, event: codex_protocol::protocol::Event) {
        match event.msg {
            codex_protocol::protocol::EventMsg::ScratchpadUpdate(update) => {
                self.on_scratchpad_update(update);
            }
            codex_protocol::protocol::EventMsg::PlanUpdate(update) => {
                self.on_plan_update(update);
            }
            _ => {}
        }
    }
