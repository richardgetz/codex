...
impl ChatWidget {
    ...
    pub(super) fn initial_collaboration_mask(
        ...
        let collaboration_modes_config = config.collaboration_modes_config();
        ...
        if mask.mode == Some(ModeKind::Plan)
            && let Some(effort) = config.plan_mode_reasoning_effort.clone()
        {
            mask.reasoning_effort = Some(Some(effort));
        }
    ...
    fn apply_thread_settings(&mut self, mut settings: ThreadSettings) {
        ...
        self.thread_usage_policy = settings.usage_policy;
