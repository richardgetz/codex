...
pub(crate) use plans::*;
pub(crate) use realtime_transcript::*;
...
impl HistoryCell for ScratchpadUpdateCell {
    ...
    fn raw_lines(&self) -> Vec<Line<'static>> {
        let mut lines = vec![Line::from("Scratchpad")];
        if self.view.show_id && !self.update.scratchpad_id.trim().is_empty() {
            lines.push(Line::from(format!("id: {}", self.update.scratchpad_id)));
        }
        if !self.update.objective.trim().is_empty() {
            lines.push(Line::from(format!("Working on: {}", self.update.objective)));
        }
        if !self.update.status.trim().is_empty() {
            let suffix = if self.update.archived_at.is_some() {
                " (archived)"
            } else {
                ""
            };
...
fn append_scratchpad_raw_section(lines: &mut Vec<Line<'static>>, label: &str, items: &[String]) {
    if items.is_empty() {
        return;
    }
