...
impl McpBinding {
    ...
    /// Returns a captured call for trusted internal extensions, including tools
    /// intentionally hidden from model-facing tool declarations.
    pub fn prepare_call_for_internal_use(
        &self,
        server: &str,
        tool: &str,
    ...
    pub fn direct_server_names(&self) -> Vec<String> {
        self.connections.direct_server_names()
    }

    pub fn lazy_server_infos(&self) -> Vec<LazyMcpServerInfo> {
        self.connections.lazy_server_infos()
    }
