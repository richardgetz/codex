...
impl McpRuntime {
    ...
    pub fn latest_direct_server_names(&self) -> Vec<String> {
        self.latest_connections().direct_server_names()
    }

    pub fn latest_lazy_server_infos(&self) -> Vec<LazyMcpServerInfo> {
        self.latest_connections().lazy_server_infos()
    }

    pub fn latest_has_enabled_server_config(&self, server_name: &str) -> bool {
        self.latest_connections()
            .has_enabled_server_config(server_name)
    }

    pub fn latest_configured_server_config_for_callable_namespace(
        &self,
        callable_namespace: &str,
    ) -> Option<(String, codex_config::McpServerConfig)> {
        self.latest_connections()
            .configured_server_config_for_callable_namespace(callable_namespace)
    }

    pub async fn latest_list_resources(
        &self,
        server: &str,
    ...
    pub async fn latest_list_resource_templates(
        &self,
        server: &str,
        params: Option<PaginatedRequestParams>,
    ...
    pub async fn latest_start_server(&self, server_name: &str) -> anyhow::Result<()> {
        self.latest_connections().start_server(server_name).await
    }
