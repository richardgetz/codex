...
async fn shutdown_cancels_pending_tool_listing() {
    ...
    manager.startup_cancellation_token.cancel();
...
async fn no_local_runtime_fails_local_stdio_but_keeps_local_http_server() {
    ...
    write_cached_codex_apps_tools_for_test(
        &codex_apps_cache_context,
        &create_test_server_info("Cached Codex Apps"),
        &cached_codex_apps_tools,
    );
    ...
    assert!(
        ...
        "lazy MCP server should not emit terminal startup events: {startup_events:?}"
    );
