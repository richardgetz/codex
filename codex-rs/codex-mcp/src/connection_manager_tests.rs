...
impl McpConnectionSet {
    ...
    fn insert_test_client(&mut self, name: impl Into<String>, client: AsyncManagedClient) {
        let name = name.into();
        let config = test_server_config(&name);
        self.servers.insert(
            name,
            McpServerView {
...
async fn list_all_tools_uses_shared_codex_apps_cache_while_client_is_pending() {
    ...
    manager
        ...
        .expect("Codex Apps test server exists")
        .config
...
#[tokio::test]
async fn list_all_tools_does_not_start_an_unstarted_lazy_server() {
    let startup_polled = Arc::new(AtomicBool::new(false));
    let startup_polled_for_future = Arc::clone(&startup_polled);
    let client = async move {
        startup_polled_for_future.store(true, Ordering::Release);
        std::future::pending::<Result<ManagedClient, StartupOutcomeError>>().await
    }
    .boxed()
    .shared();
    let approval_policy = Constrained::allow_any(AskForApproval::OnRequest);
    ...
    assert!(!startup_polled.load(Ordering::Acquire));
}

#[test]
fn startup_mode_preserves_required_eager_lazy_and_auto_browser_behavior() {
    let mut config = test_server_config("docs");
    config.startup = McpServerStartupMode::Lazy;
    ...
    assert!(should_start_server_on_session_start(
        "docs", &config, /*lazy_mcp_servers_by_default*/ true
    ));

    config.required = false;
    config.startup = McpServerStartupMode::Eager;
    assert!(should_start_server_on_session_start(
        "docs", &config, /*lazy_mcp_servers_by_default*/ true
    ));

    config.startup = McpServerStartupMode::Auto;
    assert!(should_start_server_on_session_start(
        "docs", &config, /*lazy_mcp_servers_by_default*/ false
    ));
    assert!(!should_start_server_on_session_start(
...
#[tokio::test]
async fn captured_binding_retains_hidden_internal_tool_call_authority() {
    let runtime_context = reusable_server_runtime_context();
    let config = reusable_server_config("http://127.0.0.1:1");
    let mut hidden_tool = create_test_tool("docs", "thread_hint");
    let mut meta = rmcp::model::MetaObject::new();
    meta.insert("ui".to_string(), serde_json::json!({ "visibility": [] }));
    hidden_tool.tool.meta = Some(meta);
    let manager = Arc::new(
        manager_with_reusable_ready_server(&config, &runtime_context, vec![hidden_tool]).await,
    );

    let binding = capture_binding(&manager).await;

    assert!(binding.tools().is_empty());
    assert!(binding.prepare_call("docs", "thread_hint").is_none());
    assert!(
        binding
            .prepare_call_for_internal_use("docs", "thread_hint")
            .is_some()
    );
}
