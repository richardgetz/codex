...
impl McpConnectionManager {
    ...
    pub fn has_enabled_server_config(&self, server_name: &str) -> bool {
        self.server_configs
            .get(server_name)
            .is_some_and(EffectiveMcpServer::enabled)
    }

    pub fn configured_server_config(&self, server_name: &str) -> Option<McpServerConfig> {
        self.server_configs
            .get(server_name)?
            .configured_config()
            .cloned()
    ...
    pub fn configured_server_config_for_callable_namespace(
        &self,
        ...
        self.server_configs
            ...
            .find_map(|(server_name, server)| {
                let legacy_namespace = qualified_mcp_tool_name_prefix(server_name);
                let current_namespace = legacy_namespace
                    .strip_suffix("__")
                    .unwrap_or(&legacy_namespace);
                let non_prefixed_namespace = sanitize_responses_api_tool_name(server_name);
                (callable_namespace == legacy_namespace
                    || callable_namespace == current_namespace
                    || callable_namespace == non_prefixed_namespace)
                    .then(|| {
                        server
                            .configured_config()
                            .cloned()
                            .map(|config| (server_name.clone(), config))
                    })?
            })
    }
    ...
    pub fn direct_server_names(&self) -> Vec<String> {
        let mut names = self
            ...
            .filter(|(server_name, server)| {
                server.enabled()
                    && server.configured_config().is_none_or(|config| {
            ...
            .map(|(server_name, _server)| server_name.clone())
            .collect::<Vec<_>>();
        names.sort();
        names
    }
    ...
    pub async fn list_tools_for_server(&self, server_name: &str) -> Result<Vec<ToolInfo>> {
        let client = self
            .clients
            .get(server_name)
            .ok_or_else(|| anyhow!("unknown MCP server `{server_name}`"))?;
        let managed_client = client.client().await?;
        Ok(managed_client.listed_tools())
    }
    ...
    pub async fn new(
        ...
        for (server_name, server) in server_configs
            .clone()
            .into_iter()
