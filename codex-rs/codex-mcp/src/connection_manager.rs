...
impl McpConnectionManager {
    ...
    pub fn direct_server_names(&self) -> Vec<String> {
        let mut names = self
            ...
            .keys()
            .filter(|server_name| {
                self.server_configs.get(*server_name).is_none_or(|config| {
    ...
    pub fn lazy_server_infos(&self) -> Vec<LazyMcpServerInfo> {
        let mut servers = self
            ...
            .map(|(server_name, _config)| LazyMcpServerInfo {
    ...
    pub async fn list_tools_for_server(&self, server_name: &str) -> Result<Vec<ToolInfo>> {
        ...
        let managed_client = client.client.clone().await?;
