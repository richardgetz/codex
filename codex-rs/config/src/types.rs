...
use crate::account_registry::normalize_account_alias;
...
pub use crate::mcp_types::McpServerSharingMode;
pub use crate::mcp_types::McpServerStartupMode;
...
impl From<AccountsToml> for AccountsConfig {
    fn from(toml: AccountsToml) -> Self {
        Self {
            active: toml
                .active
                .and_then(|value| normalize_account_alias(&value)),
...
fn normalize_account_rotation(rotation: Vec<String>) -> Vec<String> {
    ...
    for alias in rotation {
        let Some(alias) = normalize_account_alias(&alias) else {
            continue;
        };
        if !normalized
            .iter()
