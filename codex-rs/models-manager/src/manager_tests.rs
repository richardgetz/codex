...
fn default_collaboration_instructions(modes: &[CollaborationModeMask]) -> &str {
    modes
        .iter()
        .find(|mode| mode.mode == Some(codex_protocol::config_types::ModeKind::Default))
        .and_then(|mode| mode.developer_instructions.as_ref())
        .and_then(Option::as_deref)
        .expect("default mode should include developer instructions")
}
...
#[test]
fn static_manager_list_collaboration_modes_uses_feature_config() {
    let manager = static_manager_for_tests(ModelsResponse::default());

    let disabled = manager.list_collaboration_modes(CollaborationModesConfig::default());
    let enabled = manager.list_collaboration_modes(CollaborationModesConfig {
        default_mode_request_user_input: true,
    });

    assert!(
        default_collaboration_instructions(&disabled)
            .contains("ask the user directly with a concise plain-text question")
    );
    assert!(
        default_collaboration_instructions(&enabled)
