...
async fn plaintext_multi_agent_v2_completion_sends_agent_message() -> Result<()> {
    ...
    .await;
    let notification = "<subagent_notification>\n{\"agent_path\":\"/root/worker\",\"instruction\":\"A spawned sub-agent produced this terminal update. If it answers a user request or affects user-visible routing, relay the result to the user before reporting that there are no new supervised-session changes.\",\"parent_handling_required\":true,\"status\":{\"completed\":\"child done\"}}\n</subagent_notification>";
