...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    fixture
        .codex
        .submit(read_only_user_turn(&fixture, "call the echo tool"))
        .await?;
...
"#,
    ...
    assert!(
        responses::namespace_child_tool(&call_request.body_json(), "history", "echo").is_some(),
        "explicitly referenced history server should be exposed directly"
    );
    ...
    assert_eq!(
        actual_namespaces,
        ["mcp__other", "notes"],
        "tool_search output: {search_output:#}"
    );
