...
async fn explicitly_referenced_servers_do_not_expose_hidden_tools() {
    ...
    let runtimes = runtimes_by_name_for_input(
        &mcp_tools,
        /*connectors*/ None,
        &[],
        &explicitly_referenced_mcp_servers,
...
async fn directly_exposes_explicit_apps_without_deferred_overlap() {
    ...
    assert_eq!(
        ...
        HashMap::from([
            ...
            (
                ToolName::namespaced("mcp__rmcp", "tool_0"),
                ToolExposure::Deferred,
            ),
            (
                ToolName::namespaced("mcp__rmcp", "tool_1"),
                ToolExposure::Deferred,
            ),
        ])
    );
...
async fn defers_apps_and_non_app_mcp_tools_without_explicit_connectors() {
    ...
    let _connectors = [make_connector("calendar", "Calendar")];
    let runtimes = runtimes_by_name(
...
async fn direct_exposure_preserves_explicit_non_app_mcp_servers() {
    ...
    assert_eq!(
        ...
        HashMap::from([
            (
                ToolName::namespaced("mcp__aws_docs__", "search_documentation"),
                ToolExposure::Direct,
            ),
            (
                ToolName::namespaced("mcp__rmcp__", "tool"),
                ToolExposure::Deferred,
            ),
        ])
    );
