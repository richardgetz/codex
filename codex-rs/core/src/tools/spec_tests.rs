...
async fn spawn_agent_description_omits_usage_hint_when_disabled() {
    ...
    assert_regex_match(
        r#"(?sx)
            ...
            \s+Use\ collaboration_mode\ to\ choose\ how\ the\ child\ should\ operate:\ default\ for\ normal\ work,\ plan\ for\ planning,\ or\ orchestrator\ for\ a\ delegated\ coordinator\.
...
async fn tool_search_entries_skip_namespace_outputs_when_namespace_tools_are_disabled() {
    ...
    match &outputs[0] {
        ToolSearchEntryOutput::Tool(LoadableToolSpec::Function(tool)) => {
            assert_eq!(tool.name, "plain_dynamic")
        }
        ToolSearchEntryOutput::Tool(LoadableToolSpec::Namespace(_)) => {
            panic!("namespace tool_search output should be hidden")
        }
        ToolSearchEntryOutput::LazyMcpServer { .. } => {
            panic!("lazy MCP server output should not be present")
        }
    }
}
...
#[tokio::test]
async fn agent_browser_feature_registers_runtime_handler() {
    let config = test_config().await;
    let model_info = construct_model_info_offline("gpt-5.4", &config);
    let mut features = Features::with_defaults();
...
async fn builtin_scratchpad_can_be_disabled_for_mode() {
    ...
    .with_builtin_scratchpad_enabled(/*builtin_scratchpad_enabled*/ false);
