...
fn add_core_utility_tools(context: &CoreToolPlanContext<'_>, planned_tools: &mut PlannedTools) {
    ...
    if turn_context.tools_config.builtin_scratchpad_enabled {
        planned_tools.add(BuiltinScratchpadHandler);
    }

    if turn_context.tools_config.builtin_schedule_enabled {
        planned_tools.add(BuiltinScheduleHandler);
    }
...
fn add_mcp_runtime_tools(context: &CoreToolPlanContext<'_>, planned_tools: &mut PlannedTools) {
    let namespace_tools_enabled = namespace_tools_enabled(context.step_context.turn.as_ref());
    if let Some(mcp_tools) = context.mcp_tools {
        for tool in mcp_tools {
            match McpHandler::new(tool.clone(), namespace_tools_enabled) {
