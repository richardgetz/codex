...
fn spawn_agent_tool_description_v2(
    ...
    let agent_role_usage_hint = available_models_description
        .map(|_| {
            "Agent-role guidance above only helps choose which agent to use after spawning is already authorized; it never authorizes spawning by itself."
        })
        .unwrap_or_default();
...
If you hit an agent thread limit, close idle agents with close_agent; the user can also run /agents-prune in the owning session to close idle subagents from that session.
