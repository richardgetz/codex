...
impl ToolPlanProbe {
    fn from_router(router: ToolRouter) -> Self {
        ...
        let resolved_names = registered_tool_names
            .iter()
            .filter(|name| router.tool_runtime(name).is_some())
            .map(ToString::to_string)
            .collect::<Vec<_>>();
...
#[tokio::test]
async fn built_in_scratchpad_and_schedule_follow_mode_config() {
    let default_plan = probe(|_| {}).await;
    default_plan.assert_visible_contains(&["scratchpad"]);
    default_plan.assert_visible_lacks(&["schedule"]);
    assert!(
        default_plan
            .namespace_function_names("scratchpad")
            .iter()
    ...
    let schedule_enabled = probe(|turn| {
        update_config(turn, |config| {
            config.schedule.modes.insert(
                codex_protocol::config_types::ModeKind::Default,
                codex_config::types::ScheduleModeConfig { enabled: true },
            );
        });
        turn.tools_config = turn
            .tools_config
            .clone()
            .with_builtin_schedule_enabled(true);
    })
    .await;
    schedule_enabled.assert_visible_contains(&["scratchpad", "schedule"]);
    assert!(
        schedule_enabled
            .namespace_function_names("schedule")
            .iter()
