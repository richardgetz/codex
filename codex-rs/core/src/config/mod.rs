...
impl Config {
    ...
    pub fn auth_storage_home(&self) -> PathBuf {
        match self.active_account_alias() {
            Some(alias) => account_storage_home(&self.codex_home, alias),
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        if user_preferences_memory.enabled && memory_write_roots_enabled {
        ...
        let config = Self {
            ...
            tui_whimsy: cfg.tui.as_ref().map(|t| t.whimsy).unwrap_or(false),
