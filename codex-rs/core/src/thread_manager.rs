...
impl ThreadManager {
    ...
    pub fn list_collaboration_modes(
        ...
        self.state
            .models_manager
            .list_collaboration_modes(collaboration_modes_config)
    }
    ...
    async fn start_thread_inner(
        ...
        let inherited_exec_policy = if crate::guardian::is_guardian_reviewer_source(&session_source)
        {
            None
        } else {
            Some(Arc::new(
                ExecPolicyManager::load(&options.config)
                    .await
                    .map_err(|err| {
