...
const LEAD_MANAGER_ONLY_INSTRUCTIONS: &str = concat!(
    ...
    "Include applicable repository and skill instructions with each Worker assignment; those instructions guide the Worker’s work and do not move execution back to the Lead.\n",
    ...
    "Keep human alignment and judgment with the Lead. When a human asks about work in progress, use returned evidence or ask the responsible Worker one scoped fact/status question at the requested cadence. Do not check routine progress. After delegation, finish the assessment and park under existing wake/deadline behavior; on human guidance or actionable reports, unblock, redirect, or reassign the responsible Worker as needed. Intervene for blockers, stalled work, clearly wrong direction, or consequential decisions; otherwise leave execution with the Worker. Review each completed outcome for final acceptance and request focused corrections only for concrete gaps. Respect the configured oversight deadline; do not promise an indefinite wait. Explicit delegation restrictions, user authorization, approvals, scope, concurrency, depth, and safety rules remain binding.\n",
