...
- Use `run_policy.continuous.enabled` as the durable continuous-run switch for the current thread. When enabled and actionable `next_steps` remain, the runtime will loop back instead of allowing finalization. Move blocked work from `next_steps` to `pending_waits`; pending waits alone do not require waiting before finalization.\n\
