...
- When the user gives session-scoped safety rules for PR, merge, release, deploy, or AWS write actions, immediately persist them in `action_policy` with `set_action_policy` instead of only acknowledging them in prose. Examples include \"do not merge repo X\", \"only merge repo Y to staging/main\", or \"never deploy env prod\".\n\
...
mod tests {
    ...
    fn body_mentions_core_tools() {
        ...
        assert!(body.contains("PR, merge, release, deploy, or AWS write actions"));
    }
