...
static POWERSHELL_VERSIONS: LazyLock<Mutex<BTreeMap<PathBuf, String>>> =
...
impl EnvironmentsState {
    ...
    pub(crate) fn from_turn_context_item(
        ...
        Self {
            // Legacy turn-context rollouts only persisted a cwd, not the selected environment ID.
            // Treat those values as turn metadata, not an environment baseline, so resumed foreign
            // environments do not get a fabricated local environment that later becomes unavailable.
    ...
    for environment in &snapshot.environments {
        if let TurnEnvironmentState::Failed { selection, error } = environment {
            let detail = error
                ...
                .to_string();
            remaining_error_bytes = remaining_error_bytes.saturating_sub(detail.len());
            environments.insert(
...
fn network_from_turn_context_item(
    ...
    let codex_protocol::protocol::TurnContextNetworkItem {
        allowed_domains,
        denied_domains,
