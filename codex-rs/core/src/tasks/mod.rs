...
impl Session {
        ...
        for input in pending_input {
            match input {
                ...
                TurnInput::FunctionCallOutput(item) => {
                    response_items.push(TurnInput::FunctionCallOutput(item));
                }
                TurnInput::UserInput { .. } => {
                    warn!("dropping user input from abandoned pending-work turn");
                }
                TurnInput::InterAgentCommunication(_) => {
                    warn!("dropping inter-agent communication from abandoned pending-work turn");
                }
            }
        }
        self.input_queue
    ...
    pub async fn on_task_finished(
        ...
        let (completed_at, duration_ms, profile) = turn_context
            ...
            .await;
        let _time_to_first_token_ms = turn_context
            .turn_timing_state
