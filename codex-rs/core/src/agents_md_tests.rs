...
fn assert_invalid_utf8_warning(warnings: &[String], source: &str, path: &std::path::Path) {
    assert_eq!(warnings.len(), 1);
    assert!(
        warnings[0].contains(&format!(
            "{source} AGENTS.md instructions from `{}` contain invalid UTF-8:",
            path.display()
        )),
        "unexpected warning: {:?}",
        warnings[0]
    ...
    assert!(
        warnings[0].contains("Invalid byte sequences were replaced."),
        "unexpected warning: {:?}",
        warnings[0]
    );
}
