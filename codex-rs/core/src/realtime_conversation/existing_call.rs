...
async fn await_existing_call_step<T>(
    future: impl Future<Output = CodexResult<T>> + Send,
    stop_token: &CancellationToken,
) -> CodexResult<T>
where
    T: Send,
{
    future.or_cancel(stop_token).await?
}
...
pub(super) async fn attach(attachment: ExistingCallAttachment) -> CodexResult<JoinHandle<()>> {
    ...
    let sideband_headers = await_existing_call_step(
        model_client.realtime_sideband_headers(extra_headers),
        &stop_token,
    )
    .await?;
    let transcript_state = RealtimeTranscriptState::default();
    let connection = await_existing_call_step(
        async {
            client
