...
impl AgentControl {
        ...
        let new_thread = match (session_source, options.fork_mode.as_ref(), inheritance) {
            ...
            (Some(session_source), None, inheritance) => {
                let initial_collaboration_mode = self
                    ...
                    .await?;
                let forked_from_thread_id = session_source.parent_thread_id();
                state
    ...
    async fn spawn_forked_thread(
        ...
        if preserve_turn_context
            ...
            && let Some(usage_hint_text) = config.multi_agent_v2.subagent_usage_hint_text.clone()
