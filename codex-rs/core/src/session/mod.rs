...
impl Session {
    ...
    async fn apply_rollout_reconstruction(
        ...
        let world_state_baseline = if world_state_baseline.is_some() {
            ...
            reconstructed_environment_context_matches_current(&history, &world_state)
    ...
    pub(crate) async fn update_settings(
        ...
        let (previous_config, previous_permission_profile, previous_collaboration_mode, updated) = {
            let state = self.state.lock().await;
    ...
    async fn build_initial_context_with_world_state_and_mcp(
        ...
        if !separate_guardian_developer_message {
            ...
            let mcp_connection_manager = mcp.manager();
            if turn_context.config.include_apps_instructions && turn_context.apps_enabled() {
                let mcp_tools = mcp_connection_manager.list_all_tools().await;
                let accessible_connectors =
                    connectors::accessible_connectors_from_mcp_tools(&mcp_tools);
                let accessible_and_enabled_connectors =
                    connectors::with_app_enabled_state(accessible_connectors, &turn_context.config);
        ...
        if turn_context.config.include_skill_instructions {
            let mut filtered_skill_outcome = turn_context.turn_skills.snapshot.outcome().clone();
            filtered_skill_outcome.skills = crate::skills::filter_skills_for_mode(
                &turn_context.config,
                turn_context.mode,
                &filtered_skill_outcome.skills,
            )
            .into_iter()
            .cloned()
            .collect();
