...
fn reconstructed_environment_context_matches_current(
    ...
    let Some(latest_environment_context) = history.iter().rev().find_map(|item| {
        ...
        content.iter().find_map(|content| {
            ...
            EnvironmentsState::matches_text(text).then_some(text)
        })
    }) else {
        return false;
    };
...
const CONTEXT_WINDOW_HINTS_MAX_TOKENS: usize = 8_000;
...
impl Session {
    ...
    async fn spawn_internal(args: SessionSpawnArgs) -> CodexResult<(Arc<Self>, SessionIo)> {
        ...
        } else if crate::guardian::is_basic_session_source(&session_source) {
            let managed_policy = config
                .config_layer_stack
                .requirements()
                .exec_policy
        ...
        let collaboration_mode = initial_collaboration_mode
            .or_else(|| persisted_collaboration_mode.cloned())
            .map_or_else(
                || CollaborationMode {
...
impl Session {
    ...
    pub(crate) async fn ensure_collaboration_mode_control(
        ...
        if control.released_at.is_some() || control.mode != StateThreadControlMode::Router {
            return Ok(());
        }
                ...
                .await
        {
            warn!(
                ...
                "failed to release router collaboration mode control"
            );
    ...
    async fn apply_orchestrator_mode_overrides(
        &self,
        _previous_collaboration_mode: &CollaborationMode,
    ...
    async fn release_active_orchestrator_control(&self) {
        ...
        if control.released_at.is_some()
            ...
            || !control.reason.contains("Orchestrator collaboration mode")
        {
            return;
        }
    ...
    async fn record_initial_history(&self, conversation_history: InitialHistory) {
        ...
        let persist_inherited_usage_policy = is_subagent
            && matches!(
        ...
        let turn_context = match conversation_history {
            InitialHistory::New | InitialHistory::Cleared => {
                ...
                self.set_previous_turn_settings(/*previous_turn_settings*/ None)
                    .await;
                if persist_inherited_usage_policy {
                    // Fresh spawned children do not have a copied history prefix to carry the
                    // parent's settings event. Persist the inherited snapshot now so a cold
                    // resume keeps the effective policy even when no settings override occurs
    ...
    async fn update_settings_if(
        ...
        let (
            ...
            let root_usage_policy_changed =
                updated.parent_thread_id.is_none() && usage_policy_changed;
    ...
    pub(crate) async fn user_instructions(&self) -> Option<codex_extension_api::Instructions> {
        self.inherited_instructions().await.user
    }
                ...
                .map(|content| {
                    ContentItemKind(
                        match content {
                            ...
                            ContentItem::EncryptedContent { .. } => "user.encrypted_content",
                        }
                        .to_string(),
                    )
                })
    ...
    async fn build_initial_context_with_world_state_impl(
        ...
        if !separate_guardian_developer_message && turn_context.config.conventional_commits.enabled
        {
            developer_sections.push(ConventionalCommitsInstructions::new().render_fragment());
        }
        if !separate_guardian_developer_message && turn_context.config.git_intent_notes.enabled {
            developer_sections.push(GitIntentNotesInstructions::new().render_fragment());
        }
        ...
        if turn_context.config.features.enabled(Feature::MemoryTool)
            && turn_context.config.memories.use_memories
            && let Some(memory_prompt) =
                build_memory_tool_developer_instructions(&turn_context.config.codex_home).await
        {
            developer_sections.push(DeveloperInstructions::new(memory_prompt).render_fragment());
        }
        ...
        {
            developer_sections
                .push(DeveloperInstructions::new(user_preferences_memory_prompt).render_fragment());
        }
        if !separate_guardian_developer_message
            && turn_context
                .config
                .scratchpad
                .for_mode(turn_context.mode)
                .enabled
        {
            developer_sections.push(
                ScratchpadInstructions::new(turn_context.config.scratchpad.fanout)
                    .render_fragment(),
            );
            ...
            {
                developer_sections.push(active_scratchpad.render_fragment());
            }
        }
        if !separate_guardian_developer_message && turn_context.config.session_tmp.enabled {
            developer_sections.push(SessionTmpInstructions.render_fragment());
        }
        if !separate_guardian_developer_message
            && turn_context
                .config
                ...
                .enabled
        {
            developer_sections.push(ScheduleInstructions::new().render_fragment());
        }
        if !separate_guardian_developer_message
            && turn_context.config.situational_requirements.enabled
        {
            developer_sections.push(
        ...
        if !separate_guardian_developer_message {
            ...
            let direct_mcp_server_names = mcp
                ...
                .into_iter()
                .filter(|server_name| {
                    mcp_server_allowed_in_mode(&turn_context.config, turn_context.mode, server_name)
                })
                .collect::<Vec<_>>();
            let lazy_mcp_server_names = filter_lazy_mcp_servers_for_mode(
                &turn_context.config,
                turn_context.mode,
                &mcp.map(McpBinding::lazy_server_infos)
                    .unwrap_or_else(|| mcp_runtime.latest_lazy_server_infos()),
            )
            .into_iter()
            ...
            {
                developer_sections.push(mcp_instructions.render_fragment());
            }
        }
            ...
            && model_context_window.is_some()
        {
                ...
                .is_some_and(|config| config.use_history_notes_extension)
            {
                let mcp_result = if let Some(mcp) = mcp {
                    match mcp.prepare_call_for_internal_use("notes", "thread_hint") {
                        Some(call) => call
                            .call(
                                /*arguments*/ None,
                ...
                if let Some(mcp_result) = mcp_result.and_then(|result| {
                    let text = result
                        .content
                        .iter()
                        .filter_map(|content| {
            ...
            separate_developer_sections.push(
                crate::context::TokenBudgetContext::new(
                    ...
                    (!context_window_hints.is_empty()).then(|| {
                        truncate_text(
                            &context_window_hints.join("\n"),
                            TruncationPolicy::Tokens(CONTEXT_WINDOW_HINTS_MAX_TOKENS),
                        )
                    }),
                )
                .render_fragment(),
    ...
    pub(crate) async fn record_context_updates_and_set_reference_context_item(
        ...
        if !context_items.is_empty() {
            self.record_conversation_items(turn_context, turn_context.model_info(), &context_items)
                .await;
        }
    ...
    pub(crate) async fn record_step_environment_context_if_changed(
        ...
        self.record_context_updates_and_set_reference_context_item(step_context)
            .await
            .expect("recording step environment context should succeed")
    }
