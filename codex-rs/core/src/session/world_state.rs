...
impl Session {
    ...
    pub(crate) async fn build_world_state(&self, turn_context: &TurnContext) -> WorldState {
        self.build_world_state_for_turn_context(turn_context).await
    }
    ...
    pub(crate) async fn build_world_state_for_step(
        ...
        let model_instructions = step_model_info.get_model_instructions(step_personality);
        ...
        if turn_context.config.include_permissions_instructions {
            ...
            let model_messages = step_model_info.model_messages.as_ref();
