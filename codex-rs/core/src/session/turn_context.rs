...
impl Session {
    ...
    pub(crate) async fn new_turn_with_sub_id(
        ...
        let (previous_session_configuration, applied) = {
            let state = self.state.lock().await;
            let applied = state.session_configuration.clone().apply(&updates);
            (state.session_configuration.clone(), applied)
        };

        let next = match applied {
            Ok(next) => {
                self.apply_orchestrator_mode_overrides(
                    &previous_session_configuration.collaboration_mode,
                    next,
                )
                .await
            }
        ...
        let (previous_cwd, permission_profile_changed, codex_home, session_source) = {
            let mut state = self.state.lock().await;
            let previous_cwd = previous_session_configuration.cwd.clone();
            let previous_permission_profile = previous_session_configuration.permission_profile();
