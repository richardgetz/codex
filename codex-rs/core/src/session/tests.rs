...
async fn session_configuration_apply_rebinds_symbolic_profile_to_updated_workspace_roots() {
    ...
    session_configuration.environments = local_selections(old_root.clone());
    ...
    let updated = session_configuration
        .apply(&SessionSettingsUpdate {
            environments: Some(local_selections(new_root.clone())),
            permission_profile: Some(permission_profile),
...
async fn session_configuration_apply_retargets_implicit_workspace_root_on_cwd_update() {
    ...
    session_configuration.environments = TurnEnvironmentSelections::new(
        old_root.clone(),
    ...
    assert_eq!(
        updated.primary_workspace_roots(),
...
async fn built_tools_keeps_unavailable_mcp_placeholder_when_inventory_lists_server() {
    ...
    session
        .record_conversation_items(turn_context.as_ref(), &[previous_call])
        .await;
    let step_context = session
        .capture_step_context(Arc::clone(&turn_context))
        .await;
    ...
    .await
    .expect("build tools for explicit MCP reference");
...
async fn prepend_pending_input_keeps_older_tail_ahead_of_newer_input() {
    ...
    assert_eq!(
        drained,
        vec![
            TurnInput::ResponseItem(ResponseItem::from(blocked)),
            TurnInput::ResponseItem(ResponseItem::from(later.clone()))
        ]
    );
    ...
    assert_eq!(
        ...
        vec![
            TurnInput::ResponseItem(ResponseItem::from(later)),
            TurnInput::ResponseItem(ResponseItem::from(newer))
        ]
    );
}
