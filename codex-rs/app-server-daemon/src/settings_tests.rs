...
async fn remote_control_save_preserves_updater_settings() {
    ...
    tokio::fs::write(
        &path,
        r#"{"remoteControlEnabled":true,"shutdownGraceSeconds":25,"updater":{"autoUpdateEnabled":false,"updateIntervalMinutes":17,"futureUpdaterSetting":true},"featureOverrides":{"api_key_model_discovery":true,"code_mode_host":false},"futureSetting":42}"#,
    )
    .await
