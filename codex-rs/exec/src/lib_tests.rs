...
fn active_profile_selection_uses_profile_id_only() {
    let selection = permission_profile_selection_from_active_profile(ActivePermissionProfile::new(
        BUILT_IN_PERMISSION_PROFILE_WORKSPACE,
    ));

    assert_eq!(
        selection,
        codex_app_server_protocol::PermissionProfileSelectionParams::new(
            BUILT_IN_PERMISSION_PROFILE_WORKSPACE
        )
    );
}
