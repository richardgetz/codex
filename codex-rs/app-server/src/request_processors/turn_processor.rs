...
struct ThreadEnvironmentOverride {
    ...
    // Default-environment updates replace the separately persisted root selection.
...
impl TurnRequestProcessor {
    ...
    fn normalize_collaboration_mode(
        ...
        if collaboration_mode.settings.developer_instructions.is_none()
            && let Some(instructions) =
                builtin_collaboration_mode_presets(self.config.collaboration_modes_config())
                    .into_iter()
    ...
    async fn turn_start_inner(
        ...
        if !turn_has_input && params.tool_output.is_none() && additional_context.is_empty() {
    ...
    async fn build_environment_override(
        ...
        let workspace_roots = match workspace_roots {
            ...
            None => {
                ...
                for root in snapshot.runtime_workspace_roots.unwrap_or_default() {
    ...
    async fn thread_activity_pause_inner(
        ...
        let captured = state_db
            ...
            .map_err(|err| {
                internal_error(format!(
                    "failed to persist Team activity pause snapshot for {root_thread_id}: {err}"
                ))
            })?;
        if !captured {
            return Err(invalid_request(format!(
                "Team activity changed while capturing pause snapshot for {root_thread_id}; retry /pause"
            )));
        }
    ...
    async fn thread_activity_continue_inner(
        ...
        if let Err(error) = self
            .thread_manager
            .restore_paused_team(root_thread_id, &pause_snapshot)
            .await
        {
        ...
        let recoverable_thread_ids = persisted_plan
            ...
            .collect::<HashSet<_>>();
        let blockers = self
            .unrecoverable_activity_blockers(&pause_snapshot_thread_ids, &recoverable_thread_ids)
            .await;
