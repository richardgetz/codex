...
impl AccountRequestProcessor {
    ...
    pub(crate) async fn bedrock_setup(
        ...
        if matches!(&params, BedrockSetupParams::AccessKeys { .. }) {
            return Err(invalid_request(
                "AWS access-key setup is not supported by account/bedrock/setup; use account/login/start with amazonBedrockAccessKeys instead.",
            ));
        }
        ...
        let profile = match &params {
            ...
            BedrockSetupParams::AccessKeys { .. } => {
                unreachable!("access-key setup is rejected above")
            }
        };
