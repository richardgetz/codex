...
pub(crate) async fn apply_bespoke_event_handling(
    ...
    match msg {
        ...
        EventMsg::ScratchpadUpdate(event) => {
            handle_turn_scratchpad_update(conversation_id, &event_turn_id, event, &outgoing).await;
        }
...
mod tests {
    ...
    fn test_thread_list_state_permit() -> Arc<Semaphore> {
    ...
    async fn test_handle_turn_scratchpad_update_emits_notification_for_v2() -> Result<()> {
        ...
        handle_turn_scratchpad_update(conversation_id, "turn-123", update, &outgoing).await;

        let msg = recv_broadcast_message(&mut rx).await?;
        match msg {
            OutgoingMessage::AppServerNotification(envelope) => {
                let ServerNotification::TurnScratchpadUpdated(n) = envelope.notification else {
                    panic!("unexpected notification");
                };
                assert_eq!(n.thread_id, conversation_id.to_string());
                assert_eq!(n.turn_id, "turn-123");
