...
impl Default for ConnectionSessionState {
    fn default() -> Self {
        Self::new(crate::transport::ConnectionOrigin::InProcess)
    }
}
...
impl MessageProcessor {
    ...
    async fn handle_initialized_client_request(
        ...
        let result: Result<Option<ClientResponsePayload>, JSONRPCErrorError> = match codex_request {
            ...
            ClientRequest::ThreadScratchpadContinuousPolicySet { params, .. } => {
                self.thread_processor
            ...
            ClientRequest::ThreadControlSet { params, .. } => {
                self.thread_processor.thread_control_set(params).await
            }
            ClientRequest::ThreadControlRelease { params, .. } => {
                self.thread_processor.thread_control_release(params).await
            }
