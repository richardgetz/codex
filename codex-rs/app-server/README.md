...
- `thread/start`, `thread/resume`, and `thread/fork` accept `userPreferencesMemoryPolicy` to set the session-local read/write bucket policy for canonical `user_preferences_memory`. Responses echo the active `userPreferencesMemoryPolicy`.
...
- `thread/userPreferencesMemoryPolicy/set` — set a loaded thread’s session-local `user_preferences_memory` read/write bucket policy; returns `{}` on success. This affects later turns in that live session and does not edit `config.toml`.
...
`user_preferences_memory` buckets a loaded thread may read from or write to. The
bucket names are `durable_preference`, `personal_context`,
`relational_attunement`, `operator_playbook`, `ongoing_threads`, and
`followup_state`.

```json
{ "method": "thread/userPreferencesMemoryPolicy/set", "id": 27, "params": {
    "threadId": "thr_123",
    "policy": {
