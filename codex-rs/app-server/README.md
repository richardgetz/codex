...
The app-server streams JSON-RPC notifications while a turn is running. Each turn emits `turn/started` when it begins running and ends with `turn/completed` (final `turn` status). Token usage events stream separately via `thread/tokenUsage/updated`; their `usageByServiceTier` map buckets accumulated usage by request service tier, using `standard` for default routing and request tier ids such as `priority` for Fast mode. The payload also includes `usageByServiceTierAndContextLength`, nested by the same service-tier ids and by `short` or `long` context buckets. Each response is classified from its exact upstream `input_tokens` count using the public pricing boundary (long context is more than 272K input tokens), so clients can apply the matching rate. Older persisted payloads may omit the nested map and should use the short-context fallback. Clients subscribe to the events they care about, rendering each item incrementally as updates arrive. The per-item lifecycle is always: `item/started` → zero or more item-specific deltas → `item/completed`.
