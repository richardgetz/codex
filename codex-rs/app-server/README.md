...
- `thread/scratchpad/continuousPolicy/set` — set the loaded thread’s scratchpad-backed continuous run policy to enabled or disabled; returns `{}` after the scratchpad policy is persisted and emits `turn/scratchpad/updated`. When enabled and the scratchpad still has unfinished `next_steps`, Codex loops back to continue instead of finalizing, subject to the configured `[scratchpad.loopback]` rolling-window limit (default: five loopbacks in five minutes).
...
The app-server streams JSON-RPC notifications while a turn is running. Each turn emits `turn/started` when it begins running and ends with `turn/completed` (final `turn` status). Token usage events stream separately via `thread/tokenUsage/updated`; their `usageByModel` map keeps accumulated usage separate for every model used in the thread, while `usageByModelAndServiceTierAndContextLength` provides the same model split nested by request service tier and `short` or `long` context buckets. The payload also includes `usageByServiceTier` and `usageByServiceTierAndContextLength`, which retain aggregate service-tier breakdowns using `standard` for default routing and request tier ids such as `priority` for Fast mode. Each response is classified from its exact upstream `input_tokens` count using the public pricing boundary (long context is more than 272K input tokens), so clients can apply the matching rate. Older persisted payloads may omit the model and nested maps and should use the aggregate or short-context fallback. Clients subscribe to the events they care about, rendering each item incrementally as updates arrive. The per-item lifecycle is always: `item/started` → zero or more item-specific deltas → `item/completed`.
