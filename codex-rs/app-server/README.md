...
- `server/lifecycle/read` — experimental; read this app-server process's lifecycle phase (`ready`, `draining`, or `forced`), process identity, drain transition identity, and running assistant-turn count.
- `server/lifecycle/updated` — experimental; receive a best-effort delivery of the same process-local lifecycle snapshot when graceful drain starts, is forced, or its running assistant-turn count changes while draining. The server waits briefly for each opted-in writer before shutdown, but cannot guarantee client consumption.
...
- `thread/eta/read` and `thread/eta/update` — experimental, root-session-scoped task estimate APIs. `thread/eta/read` is read-only and returns bounded active work plus a cursor-paginated history page; it never resumes a thread, starts a turn, or infers progress. `thread/eta/update` accepts explicit `create`, `start`, `revise`, `block`, `complete`, and `cancel` operations and emits `thread/eta/updated` after a committed change. Task IDs are scoped to the root session, while child workers may update only tasks they own; create/update operations can carry a verified `ownerThreadId`, and responses expose the persisted owner and `isStale` projection state. Estimates are nullable lower/upper second ranges; the original range is retained when work starts and revisions are bounded. Completion and cancellation are explicit terminal states, blocked work remains active, and idle, turn completion, or an elapsed estimate never completes a task. History retains harness timestamps, actual elapsed seconds, and early/within/late classification against the original range; cancellation is classified as unknown. Unknown, stale, missing, and dependency-blocked work keeps the aggregate finish range unknown. Read history with `cursor` and `limit` (the server caps pages at 100). The configured `[eta].freshness_minimum_minutes` defaults to 15 minutes; the resolved root policy is persisted with the ETA ledger, cold reads seed missing policy rows from server configuration, and worker configuration refreshes cannot replace the Lead policy. Freshness and overdue reminders are event-driven one-shot messages sent directly to the persisted owner; revisions, reassignment, lifecycle, configuration, pause, shutdown, removal, and owner-lifecycle changes invalidate or re-arm stale callbacks without polling. The model-facing `update_eta` tool accepts at most eight operations and returns all changed summaries; public app-server batches may be larger. The unfinished-task cap excludes terminal History. Deleting a root removes its ETA ledger; deleting a worker preserves terminal History and marks unfinished owned tasks blocked.
- ETA timestamps remain Unix seconds in API payloads; clients render them in UTC by default and may opt into system-local display with `[eta].use_local_timezone = true` or a named IANA `[eta].timezone` override (named zones take precedence and honor DST).
...
- `thread/activity/pause` — experimental; cooperatively pause the selected thread's Lead tree, including every currently loaded ThreadSpawn descendant. When the selected thread is a Worker, its Lead root is used. The response is `{}` after Core applies the gate; model and tool work admitted after the pause waits at its next boundary, while already-running operations may finish. A successful request durably records the root's pause intent before applying the process-local gate, so a replacement or restart can restore the paused tree. This does not cancel, suspend, or replay external commands. If the state database is unavailable, the request fails instead of claiming a durable pause.
  A lifecycle drain is process-wide and independent of this pause state; it rejects new thread/turn/goal/queue/usage-resume/activity-continue/response-item-injection/review/realtime input/MCP tool/event-stream/compact/shell-command/Windows sandbox setup/process admission while existing turns drain, but it does not create a durable handoff checkpoint. This stage fences app-server request ingress and tracks direct/inline admissions; queued requests and Core-owned automatic continuations still require the shared handoff coordinator to close their internal admission paths.
- `thread/activity/continue` — experimental; reconcile open persisted Team descendants, then release the selected Lead tree's cooperative pause after Core acknowledges the gate change. It also requests an immediate usage check for any retained usage wait. The response is `{}` and no synthetic model turn is created. A failed or interrupted recovery keeps the durable pause intent and returns an actionable error; an explicit retry is required after the runtime is ready.
...
### Example: Configure and toggle Lead/Worker routing

Lead/Worker assignments are opt-in for new threads. Add both profiles to
`config.toml` while leaving `enabled = false` when clients should choose the
mode per thread:
...
and the model restored by `off` come from the server's thread state.

```json
{ "method": "thread/settings/update", "id": 14, "params": {
    "threadId": "thr_123",
    "team": { "mode": "leadWorker" }
} }
{ "id": 14, "result": {} }
...
Switching back to `off` restores the single model and effort that were active
before the thread entered team mode:

```json
{ "method": "thread/settings/update", "id": 15, "params": {
    "threadId": "thr_123",
    "team": { "mode": "off" }
} }
```
...
threads continue to use the configured default independently of another
thread's selection. This initial implementation provides hard model routing for the
configured Lead and Worker roles. Existing skill execution and Lead acceptance
guidance continue to apply; it does not add a separate tool sandbox or verify
