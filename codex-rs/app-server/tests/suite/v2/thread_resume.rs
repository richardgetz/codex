...
async fn cold_resume_permission_overrides_win_over_persisted_profile() -> Result<()> {
    ...
    for params in [
        ...
        ThreadResumeParams {
            thread_id: thread_id.clone(),
            permissions: Some(BUILT_IN_PERMISSION_PROFILE_READ_ONLY.to_string().into()),
            ..Default::default()
        },
        ThreadResumeParams {
            thread_id: thread_id.clone(),
...
#[tokio::test]
async fn thread_resume_does_not_treat_missing_legacy_effort_as_explicit_clear() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
    mock_responses_config(&server.uri()).write(codex_home.path())?;
    let config_path = codex_home.path().join("config.toml");
