...
"#,
    ...
    assert!(
        skill_fragments
            .iter()
            .all(|fragment| !fragment.contains(LOCAL_SKILL_MARKER))
    );
    let selected_skill_fragments = skill_fragments
        .into_iter()
        .filter(|fragment| fragment.contains(&format!("<name>{SKILL_NAME}</name>")))
        .collect::<Vec<_>>();
    assert_eq!(1, selected_skill_fragments.len());
    let skill_fragment = selected_skill_fragments
        .first()
        .expect("selected executor skill instructions should be model-visible");
