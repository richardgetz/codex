...
async fn thread_delete_with_non_local_thread_store_does_not_create_local_persistence() -> Result<()>
{
    ...
    assert_no_local_persistence_artifacts(codex_home.path(), &["memories"])?;
...
fn assert_no_local_persistence_artifacts(
    codex_home: &Path,
    additional_expected_entries: &[&str],
) -> Result<()> {
    ...
    // initialize shell snapshot storage, and scratchpad lifecycle cleanup may
    // leave its durable lock file. None of these are thread persistence.
    entries.remove(".sandbox_migration");
    entries.remove(".write.lock");
