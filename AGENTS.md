...
- Batch related source edits and review the diff before starting an expensive Rust command. Select the narrowest changed crate or test target first; a package/target selector (for example, `just test -p codex-tui`) chooses what to build, while a supported test-name filter only narrows tests within that selection. Do not substitute one for the other.
