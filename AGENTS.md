...
2. Start with the narrowest relevant project and build target, using an explicit supported test-name filter when one is useful. A package or target selector controls build scope; a test-name filter only narrows tests within that selection, so do not substitute one for the other. If the selected test coverage does not compile an affected consumer, run the narrowest compile or check for that consumer.
3. Broaden validation only for a concrete dependency, coverage, or diagnosed failure reason, and record why the broader scope is needed. Do not automatically run the complete workspace suite solely because common, core, or protocol changed. Run the complete suite with `just test` only after user approval. Reuse completed validation for the same source revision; do not repeat it without a source or configuration change or a diagnosed infrastructure or flaky failure. Avoid `--all-features` for routine local runs because it expands the build matrix and can significantly increase `target/` disk usage; use it only when you specifically need full feature coverage.
4. Documentation-only or formatting-only changes with no semantic effect do not require a rebuild or behavior test. A touched file alone is not a reason to rerun existing behavior tests; run a narrow check only when it can catch a relevant issue.
...
### Rust validation and long-command waiting

- Batch related source edits and review the diff before starting an expensive Rust command. Apply the validation-scope rules above when choosing package/target and test-name filters; keep expensive commands narrow until a concrete reason requires broader coverage.
