...
class InstallShTest(unittest.TestCase):
    ...
    def test_fork_release_uses_fork_repo_and_tag(self) -> None:
        fork_version = f"{VERSION}-rick.1"
        result, requests = run_installer(fork_version)

        self.assertNotEqual(result.returncode, 0)
        self.assertEqual(
            requests,
            [
                "https://api.github.com/repos/richardgetz/codex/releases/tags/"
                f"rick-v{fork_version}",
                "https://github.com/richardgetz/codex/releases/download/"
                f"rick-v{fork_version}/codex-package_SHA256SUMS",
            ],
        )
        self.assertIn(f"Resolved version: {fork_version}", result.stdout)
